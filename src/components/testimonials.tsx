        ...
        <div className=" mx-auto flex flex-col items-center  gap-8  items-start p">
          <div className="lg:flex lg:items-center lg:justify-between z-10">
            <div>
              ...
              <p className="text-white mt-4 text-center">Don&apos;t just take our word for it - here&apos;s what some of our happy clients have to say about our pet care services</p>
            </div>
          </div>
            ...
            <Slider {...settings} className="">
              {testimonials.map((testimonial, index) => (
                ...
                <div className="bg-black rounded-lg shadow-md p-6 md:max-w-[350px]  md:mx-auto h-full border border-primary">
                  <div className="flex items-center justify-center h-24 w-24 mx-auto mb-4 bg-[#343538] rounded-full">
                  </div>
                  <h3 className="text-lg font-bold text-center text-white font-heading">
                    {testimonial.name}
                  </h3>
                  <p className="text-primary font-heading mb-4 text-center font-extrabold">{testimonial.location}</p>
                  <p className="text-white text-center text-sm  md:min-h-40">{testimonial.text}</p>
                </div>
                </div>
              ))}
            </Slider>
          </div>
          <p className="text-white mt-2 text-center">(More testimonials coming soon - we&apos;re proud to have a growing list of satisfied pet parents!) These reviews are collected from our clients via email and other platforms, and we&apos;re happy to share references upon request. Your feedback means the world to us, and it helps us continually improve our services. We strive to ensure every experience is as positive as those mentioned above.
</p>
        </div>
      </div>
    </div>
